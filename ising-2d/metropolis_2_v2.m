% metropolis_2_v2.m
%
%  This is a modified version of `metropolis_2.m` that add some additional
%  scripts to see how the magnetization changes over the iteration of
%  Monte Carlo transition states.
%
%  Logs
%  [22/06/18]`
%  - Discritize the temperature by the number of ponts instead of dT
%  
%  [22/06/19]
%  - Add expected value of specific heat
%
%  [22/07/10]
%  - Revision to the calculation of specific heat
%    See `metropolis_1_v2.m`
%  - Improve the calculation of energy such that we only compute for the
%    flipped site not the entire sites.
%
%  [22/07/11]
%  - 
%
...
function simulate_IsingModel(Tf, J, Tc, Tmin, Tmax, kb, kbT, N, L, L_avg, ...
  ...
  beta_arr = 1./(kb*Tf);
  spinSum_each_state = sum(x_config_spin(:));
  ...
  for q = 1:Q
    ...
    p = [1, ...              % Delta H = -8J < 0
         ...
         exp(-4*J/kbT), ...  % Delta H = 4J
         exp(-8*J/kbT)];     % Delta H = 8J
    ...
    if mod(q, L) == 0
      ...
      %set(magnetization_plot_handler, 'XData', kbT, 'YData', abs(M)); 
