% metropolis_AF_stagM_singleT.m
%
%   This script creates a plot of calculation to the staggered
%   magnetization and the energy for antiferromagnetic Ising model.
% 
...
%% -- user-defined input
N = 50; 
L = 15000;   % number of samples after one MCS (Monte Carlo Sweep)
J = -1; 
kb = 1;
T_i = 2.1;   % Tc = 2.269
...
for l = 1:L
  ...
  if mod(l-1, 100) == 0
    M = get_spinSum_staggered(x_config_spin, N)/N^2;
...
%       j_p1 = mod(j, N) + 1;    % j+1
%       
%       spin_ij = x_config_spin(i, j);
%       spinSum_each_site(i, j) = spin_ij + (-1)*spin_ij*( ...
%         x_config_spin(i_m1, j) ...
%         + x_config_spin(i, j_m1) ...
%         + x_config_spin(i_p1, j) ...
%         + x_config_spin(i, j_p1));
